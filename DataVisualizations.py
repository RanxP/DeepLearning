...
def transform_tensor_to_class_color(target,trainId: bool = True) -> np.array:
    ...
    for label in LABELS:
        ...
        if trainId:
            im[target.squeeze() == label.trainId] = label.color
        else:
            im[target.squeeze() == label.id] = label.color
    
    return im

def transform_list_to_class_color(target,trainId: bool = True) -> np.array:
    """Visualize the cityscapes label
    input: Tensor of shape (batch_size, height, width) with values from 0 to 18
    output: None
    """
...
def visualize_criterion(baseline, prediction, loss, criterion_name:str):
    ...
    for i in range(batch_size):
        axs[0, i].imshow(transform_list_to_class_color(baseline[i],trainId=True))
        axs[0, i].set_title('Baseline')
        axs[0, i].axis("off")
        axs[1, i].imshow(transform_list_to_class_color(prediction[i],trainId=True))
